...
class GraphLevelOutput(nn.Module):
    def __init__(self, node_features, output_dim, time_steps=100, inner_prod="default", activation_fn=None, attn_act=None, dropout=0):
        ...
        i = nn.Linear(self.in_dim, self.out_dim, bias=False)

        j = nn.Linear(self.in_dim, self.out_dim, bias=False)
    ...
    def forward(self, input_set, mask=None, mprev=None, cprev=None):
        ...
        if mask is not None:
            ...
            gated_activations = torch.sigmoid(self.i(input_set)) * self.j(input_set) * mask
            # gated_activations = torch.sigmoid(self.i(input_set * mask)) * self.j(input_set * mask) * mask
