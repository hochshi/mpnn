...
try:
    ...
    data = file_data['data']
    for graph in data:
        graph.mask = from_numpy(np.ones(graph.afm.shape[0], dtype=np.float32).reshape(graph.afm.shape[0], 1)).half()
        graph.afm = from_numpy(graph.afm.astype(np.float32)).half()
        graph.bfm = from_numpy(graph.bfm.astype(np.float32)).half()
        graph.adj = from_numpy(graph.adj.astype(np.float32)).half()
